...
const Register = () => {
  ...
  const sureHandle = async () => {
    ...
    console.log(values)
    Apis.registerFromPhone(values).then(res => {
  ...
  return <div className='register-wrapper'>
    ...
    <div className='register-content2'>
      <Form requiredMarkStyle='asterisk' className='content-wrapper' form={form}>
        {/* <Form.Item name='name' label={<span className='content-item-title user-title'>用户名</span>} rules={[{ required: true }]}>
          <Input placeholder='请输入姓名' className='input-style' />
        </Form.Item> */}
        ...
        <Form.Item name='passwd' label={<span className='content-item-title password-title'>密码</span>} rules={[
        ...
        <div className='register-btn2' onClick={sureHandle}>
          <span>确定</span>
        </div>
      </Form>
    </div>
