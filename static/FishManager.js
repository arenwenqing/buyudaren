...
(function () {
  ...
  FishManager.prototype.makeFish = function () {
    ...
    var type = ns.R.fishTypes[index]; // index
    ...
    var group = this.fishPool.splice(0, num),
      cy = game.height >> 1;
    var typeWidth = type.frames[0].rect[2],
      typeHeight = type.frames[0].rect[3];
    // var sx = Math.random() > 0.5 ? -typeWidth : game.width + typeWidth;
    // var sy = Math.random()* 200 + (game.height >> 1) - 100 >> 0;
    // var sx =  Math.random() * (game.width + typeWidth * 2) - typeWidth - 100
    var sx = Math.random() * game.width;
    var sy = Math.random() > 0.5 ? -typeHeight : game.height + typeHeight;
    var speed =
      Math.random() * (type.mixin.maxSpeed - type.mixin.minSpeed) +
      type.mixin.minSpeed;
    var degree = sy > 0 ? -90 : 90; // (Math.random() * 20 - 10 >> 0);
    if (sx + typeHeight >= game.width) {
      sx = game.width / 2;
    }
    //make fish alive
    for (var i = 0; i < num; i++) {
    ...
    ns.FishGroup.setRandomPatten(group, sx, sy, game.width);
  };
})();
